...
impl BitmapMeta {
  ...
  pub const fn new(expire_at: u64, version: u64, size: u64) -> Self {
    Self {
      base: KeyMeta::new(RedisType::Bitmap, expire_at, version, size),
    }
  }
  ...
  pub fn new_with_version(expire_at: u64, size: u64) -> Self {
    Self {
      base: KeyMeta::new_with_version(RedisType::Bitmap, expire_at, size),
    }
  }
  ...
  pub fn decode(bytes: &[u8]) -> Option<Self> {
    ...
    if base.rtype == RedisType::Bitmap {
      Some(Self { base })
    } else {
      None
    }
...
impl MetaOps for BitmapMeta {
  const TAG: &[u8] = KeyTag::BitmapMeta.as_slice();
