...
impl SetMeta {
  ...
  pub const fn new(expire_at: u64, version: u64, size: u64) -> Self {
    Self {
      base: KeyMeta::new(RedisType::Set, expire_at, version, size),
    }
  }
  ...
  pub fn new_with_version(expire_at: u64, size: u64) -> Self {
    Self {
      base: KeyMeta::new_with_version(RedisType::Set, expire_at, size),
    }
  }
...
impl MetaOps for SetMeta {
  const TAG: &[u8] = KeyTag::SetMeta.as_slice();
